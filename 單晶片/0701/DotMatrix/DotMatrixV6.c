// PROGRAM	: DotMatrixV6.c							2019.0307
...
//const u8 string[]={"\0x80\0x81\0x082"};
// ----- 要顯示的中文字串 (使用 0x80~0x85 索引) -----
// 0x80=鄭, 0x81=立, 0x82=強, 0x83=的, 0x84=程, 0x85=式
const u8 string[] = { 0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x00 };
...
{	const u8 *pstr;
	...
	{	pstr=string;
		...
		{	for(line=0;line<8;line++)
			{	Buffer[15]=font[*pstr-' '][7-line];//複製字型
				for(i=0;i<(8-line);i++)
				{	ShiftDn(7-line);
...
void ShiftDn(u8 index)
{	u8 i;
	for(i=index;i!=0;i--) Buffer[i]=Buffer[i-1];	//Buffer Shift
	Buffer[0]=Buffer[15];							//Reload temp to Buffer[0]
	Buffer[15]=0;
}
void DisplayChar(u8 ms)
