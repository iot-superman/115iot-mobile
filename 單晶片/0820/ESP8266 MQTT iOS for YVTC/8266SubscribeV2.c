// PROGRAM	: 8266SubscribeV2.c	(RGB LED Controll)					2024.0506
...
const s8* filter1="/YVTC006/led/#";    								//Subscribe Topic,/YVTCxxx/led/下所有主題，修正xxx為自己的學號
#define	SENSOR1	"/YVTC006/led/r"									//感測器名稱(1)，修正006為自己的學號
#define	SENSOR2	"/YVTC006/led/g"									//感測器名稱(2)，修正006為自己的學號
#define	SENSOR3	"/YVTC006/led/b"									//感測器名稱(3)，修正006為自己的學號
#define SensorCNT 3
...
//		GCC_NOP(); 
	...
	ptr=(u8*)strstr((char*)RxBuf0,(char*)":")+5;							//ptr指向: (+IPD,006:CMD_LEN_HPID_LPID_ReturnCode)
		...
		{	LEDControl(1);													//若有收到
			...
			{	case PUBLISH:
					...
					{	ptr=myFindStr((u8*)RxBuf0,(u8*)&sensorID[i][0],RxCount);
						...
						switch(i)
						{	case 0: pRLED=(*ptr=='1') ? LEDOn : LEDOff;
									break;
							case 1: pGLED=(*ptr=='1') ? LEDOn : LEDOff;
									break;
							case 2: pBLED=(*ptr=='1') ? LEDOn : LEDOff;
									break;
						}
						if(*ptr!='0') eDisplayRet(75,24,59+i*20,16,colorTab[i]);
						else eDisplayRet(75,24,59+i*20,16,BackColor);
					}
			}
		}
