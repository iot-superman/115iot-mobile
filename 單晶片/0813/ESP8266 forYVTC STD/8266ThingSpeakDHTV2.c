...
/*
#define	SSID	"thmrb306"										//改成自己的手機熱點
#define	PSWD	"thmrbthmrb"									//改成自己的手機熱點密碼
#define 	ConIP	"api.thingspeak.com"							//欲連線的IP
#define 	UPDATE	"GET /update?key=!!!!!!!!!!!!!!"				//改成自己的Channel Write API Key
*/

#define	SSID	"Hot2"										//改成自己的手機熱點
#define	PSWD	"12345678"									//改成自己的手機熱點密碼

#define ConIP	"api.thingspeak.com"								//欲連線的IP
#define UPDATE	"GET /update?key=GQMVIJSCF02U11AU"				//改成自己的Channel Write API Key
...
//		GCC_NOP(); 
	...
	{	for(i=0;i<255;i+=8)
		{	
			// ===== Debug：準備連線 ThingSpeak =====
			  eDisplayStr(10,115,(u8*)"TCP...");
			
			if(SetUpConnection(0,0,(u8*)"TCP",(u8*)ConIP,80)!=0)
			
			   // ===== Debug：TCP 連線成功 =====
                eDisplayStr(10,115,(u8*)"TCP OK ");
