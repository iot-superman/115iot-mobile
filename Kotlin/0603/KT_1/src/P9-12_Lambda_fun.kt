 ...
 */
    ...
    runSimulate("Mary", funTwoParam = { playerName: String, age: Int ->
        "玩家名稱: $playerName, 年齡: $age" }
    )

    //call runSimlate1 demo
    //這裡的 it 是 Lambda 的參數，這個 Lambda 的回傳值是 it * 2 的結果，也就是 10
    //Lambda 的參數類型可在最後才可以用這種寫法，因為 Lambda 的參數類型可以從 runSimulate1 函式的定義中推斷出來，所以不需要在 Lambda 的參數列表中指定類型了
    //不是在最後一個參數的 Lambda 就不能用這種寫法了，因為 Lambda 的參數類型無法從函式的定義中推斷出來了，所以需要在 Lambda 的參數列表中指定類型了
    runSimlate1({ it * 2 }, "John", 10)

    //只可以用 age 來當 Lambda 的參數名稱，不能用 it 來當 Lambda 的參數名稱了，因為這個 Lambda 不是在最後一個參數的位置了，
    // 所以 Lambda 的參數類型無法從函式的定義中推斷出來了，所以需要在 Lambda 的參數列表中指定類型了
    var funOneParm : (Int) -> Int = {age-> age*2 }
    runSimlate1(funOneParm, "John", 100)  //這裡的 age 是 Lambda 的參數，這個 Lambda 的回傳值是 age * 2 的結果，也就是 200


    println()

    //這裡的 name 和 age 是 Lambda 的參數，這個 Lambda 的回傳值是 println("玩家名稱: $name, 年齡: $age") 的結果，也就是 Unit
    runSimlate2("Henry", dataPrint = {name,age->println("玩家名稱: $name, 年齡: $age")}){
        it + 10
    }
}
...
fun runSimlate2(name : String, dataPrint:(String,Int)->Unit, funOne:(Int)-> Int)
{
...
fun runSimlate1(funOneParm:(Int)-> Int,name:String, num:Int) :Unit
{
    val value=funOneParm(num)
    println("玩家名稱: $name, has $value")
}

//這裡的 funTwoParam 是 Lambda 的匿名函式的參數名稱，在最後面所以可以直接使用 Lambda 的內容來定義這個 override 的 Lambda函數
fun runSimulate(
    playerName: String,
    funTwoParam: (String, Int) -> String   //這裡的 (String, Int) -> String 是 Lambda 的類型(匿名函式的類型)，表示這個 Lambda 接受兩個參數，第一個參數是 String 類型，第二個參數是 Int 類型，並且回傳一個 String 類型的結果，這裡的 funTwoParam 就是這個 Lambda 的參數名稱，可以在 Lambda 的內容中使用它來代表這個 Lambda 的參數，這樣就可以讓 runSimulate 函式更具有彈性和可重用性，因為我們可以傳遞不同的 Lambda 來實現不同的邏輯，而不需要修改 runSimulate 函式的內容
) {
    val numBuilding = (1..10).shuffled().last()
    println(funTwoParam(playerName, numBuilding)) //這裡的 funTwoParam 是 Lambda 的寫的匿名函數
}
